...
Detector::Detector(ros::NodeHandle &nh) : nh_(nh) {
  pub_yellow_pixels_ =
      nh.advertise<std_msgs::UInt32MultiArray>("yellow_pixels", 0);
...
void Detector::RGBCallback(const sensor_msgs::ImageConstPtr &RGB_img) {
  ...
  for (size_t i = 0; i < pixel_yellow_.size(); i++) {
    // ROS_INFO("[%u] [%u]
    // [%u]",image_data[pixel_yellow_[i-1]],image_data[pixel_yellow_[i]],image_data[pixel_yellow_[i+1]]);
    if (image_data[pixel_yellow_[i] - 1] > 200 &&
        image_data[pixel_yellow_[i]] < 80 &&
        image_data[pixel_yellow_[i + 1]] < 80) {
      // ROS_INFO("Red Light!");
      msg_traffic_state.data = true;
      pub_traffic_state_.publish(msg_traffic_state);
    } else if (image_data[pixel_yellow_[i] - 1] < 100 &&
               image_data[pixel_yellow_[i]] > 200 &&
               image_data[pixel_yellow_[i + 1]] < 100) {
