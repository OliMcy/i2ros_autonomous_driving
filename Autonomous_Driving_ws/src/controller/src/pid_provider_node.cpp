...
class PidProviderNode {
  ...
  ros::Subscriber current_twist_sub;
...
public:
  PidProviderNode() {
    target_twist_sub = nh.subscribe("target_velocity", 1,
                                    &PidProviderNode::extractPidTarget, this);
    current_twist_sub = nh.subscribe(
        "current_velocity", 1, &PidProviderNode::extractCurrentTwist, this);
  ...
  void extractCurrentTwist(const geometry_msgs::Twist twist) {
    std_msgs::Float64 v;
    v.data = twist.linear.x;
