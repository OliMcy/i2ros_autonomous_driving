...
public:
  ...
  void onCurrentCmdVel(const geometry_msgs::Twist& cmd_velocity){
      ...
      // ROS_INFO_STREAM(cmd_vel_linear_old[0]);
  }
  ...
  void controlLoop(const ros::TimerEvent& t){
    ...
    // msg.angular_velocities[0] = -(vel_ist[0]-cmd_vel_linear_current[0])/1; // Acceleration
    // msg.angular_velocities[1] = -cmd_vel_angular[2];  // Turning angle
    // msg.angular_velocities[2] = 0; // Breaking
    // msg.angular_velocities[3] = 0;

    msg.angular_velocities[0] = -(vel_ist[0]-ackermann_cmd_vel); // Acceleration
    msg.angular_velocities[1] = -ackermann_cmd_steering_angle;  // Turning angle
    msg.angular_velocities[2] = 0; // Breaking
    msg.angular_velocities[3] = 0;
    // ROS_INFO_STREAM(vel_ist[0]<<vel_ist[1]<<vel_ist[2]);
    ROS_INFO_STREAM(-(vel_ist[0]-ackermann_cmd_vel)/1);
